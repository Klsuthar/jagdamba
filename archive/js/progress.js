const CLASS_CONFIGS = [
    ...
    { number: 5, classKey: 'class_5', configKey: 'class5', className: 'Class 5', idPrefix: 'STU5_' }
];
...
// Excel data source path
const EXCEL_FILE_PATH = '../data/Student_List_All_Classes.xlsx';
...
/**
 * Load students from Excel file using SheetJS
 * Falls back to JSON if Excel is not available or SheetJS not loaded
 */
async function loadStudentsFromExcel() {
    // Check if SheetJS is available
    if (typeof XLSX === 'undefined') {
        console.warn('SheetJS not loaded, falling back to JSON');
        return false;
    }

    try {
        const response = await fetch(`${EXCEL_FILE_PATH}?v=${Date.now()}`);
        if (!response.ok) {
            console.warn('Excel file not found, falling back to JSON');
            return false;
        }
        ...
        CLASS_CONFIGS.forEach(cfg => {
            const sheetName = EXCEL_SHEET_NAMES[cfg.classKey];
            // Try exact name, then try case-insensitive match
            let sheet = workbook.Sheets[sheetName];
            if (!sheet) {
                const matchedName = workbook.SheetNames.find(
                    n => n.toLowerCase().replace(/\s+/g, '') === sheetName.toLowerCase().replace(/\s+/g, '')
                );
                if (matchedName) {
            ...
            const students = rows
                .map(row => {
                    const rollNo = normalizeRollNo(row.roll_no || row.Roll_No || row['Roll No'] || row.rollno || '');
                    if (!rollNo) return null;

                    const studentName = row.student_name || row.Student_Name || row['Student Name'] || row.name || 'Student';
                    const fatherName = row.father_name || row.Father_Name || row['Father Name'] || '';
                    const motherName = row.mother_name || row.Mother_Name || row['Mother Name'] || '';
                    const dob = row.dob || row.DOB || row['Date of Birth'] || '';
                    const image = row.image || row.Image || row.photo || '';
                    ...
                    return {
                        ...
                        motherName,
                        dob: String(dob),
                        photo: buildStudentPhotoPath(cfg, rollNo, image)
                    };
                })
                .filter(Boolean)
                .sort((a, b) => parseInt(a.rollNo, 10) - parseInt(b.rollNo, 10));
            ...
            if (students.length > 0) {
                classData[cfg.classKey].students = students;
            }
        });

        console.log('Students loaded from Excel successfully');
        return true;
    } catch (error) {
        console.warn('Error loading Excel file:', error);
        return false;
    }
}
...
async function loadStudentData() {
    try {
        ...
        // Try loading students from Excel first
        markLoadingStep('Trying Excel data source...');
        const excelLoaded = await loadStudentsFromExcel();

        if (!excelLoaded) {
            // Fall back to JSON student files
            ...
            setLoadingTotal(1 + studentRequests.length + CLASS_CONFIGS.length + examFileCount, 'Loading class records');
            ...
            setLoadingTotal(1 + CLASS_CONFIGS.length + examFileCount, 'Loading from Excel');
            markLoadingStep('Excel data loaded');
        }

        // Load attendance from JSON (always)
    ...
    if (classInfo.students.length === 0) {
        studentsGrid.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 3rem 1rem;">
                <i class="fas fa-user-clock" style="font-size: 3rem; color: rgba(255,255,255,0.3); margin-bottom: 1rem; display: block;"></i>
                <p style="color: rgba(255,255,255,0.6); font-size: 1.1rem;">No students enrolled yet for this class.</p>
                <p style="color: rgba(255,255,255,0.4); font-size: 0.9rem; margin-top: 0.5rem;">Student data will appear here once added.</p>
            </div>
        `;
    } else {
            ...
            const studentCard = `
                <div class="student-card" onclick="showStudentReport('${student.id}')">
                    <div class="student-avatar">
                        <img src="${student.photo}" alt="${student.name}" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(student.name)}&background=6366f1&color=fff&size=150'">
                    </div>
                    <h4>${student.name}</h4>
...
function displayAllExams(student, studentId, allExams) {
    ...
    photoElement.onerror = printPhotoElement.onerror = function() {
        this.onerror = null;
        this.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(student.name)}&background=6366f1&color=fff&size=200`;
    };
    ...
    currentClassStudents.forEach(student => {
        ...
        card.innerHTML = `
            <img src="${student.photo}" alt="${student.name}" 
                 onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(student.name)}&background=6366f1&color=fff&size=80'">
            <p>${student.name}</p>
        `;
